...
void	throw_ray_x(t_vect *res, t_vect ray)
{
	double		m;
	int			x;
	double		y; 
	...
	if (ray.x == 0.f)
		return ;
	if (ray.y == 0.f)
		return ; // appeler une autre fonction de calcul car cas tres specifique
	m = ray.y / ray.x;
	if (ray.x > 0)
	{
		x = (int)g_cube.curr_map.p_pos.x;
		x++;
		while (x < 10) //map_width
		{
...
void	throw_ray_y(t_vect *res, t_vect ray)
{
	double		m;
	double		x;
	int			y; 
...
void	put_coloumn_wall(t_vect *ray, t_vect *pos, int coloumn)
{
	...
	while (i < (RES_HEIGHT / 2) + (wall_size / 2))
	{
		wall_line = (int)((float)(i - ((RES_HEIGHT / 2) - (wall_size / 2))) * (float)g_cube.curr_map.textures.n_wall.img_height / (float)wall_size);
		if (i < RES_HEIGHT && i > 0 && wall_coloumn < g_cube.curr_map.textures.n_wall.img_width && wall_line < g_cube.curr_map.textures.n_wall.img_height)
			((unsigned int *)g_cube.img_onload.buffer)[i * (g_cube.img_onload.sl / 4) + coloumn] = 
				((unsigned int *)(g_cube.curr_map.textures.n_wall.buffer))[wall_line  * (g_cube.curr_map.textures.n_wall.sl / 4) + wall_coloumn];
		i++;
	}
}
...
void	raycast()
{
	...
	while (i < RES_WIDTH)
	{
		...
		throw_ray_x(&(wall_hit[i]), ray);
		throw_ray_y(&(wall_hit[i]), ray);
		//only for debbuging
		if (! (i % 50))
		...
		put_coloumn_wall(&ray, &(wall_hit[i]), i);
		i++;
	}
}